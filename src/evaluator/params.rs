...
impl FiniteParams {
    ...
    pub fn apply(&self, args: &[Arg]) -> FiniteParams {
        ...
        let str=new_params.iter().map(|x| x.to_string()).collect::<Vec<String>>().join(",");
        // println!("apply_finite:{}", str);
...
pub fn finite_params_test() {
    ...
    let args = [
        Arg::Evaluated(Num(20)),
        Arg::Evaluated(Num(30)),
        Arg::Evaluated(Num(40)),
        Arg::Evaluated(Num(50)),
    ];
